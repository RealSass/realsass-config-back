...
async function main() {
    ...
    for (const theme of themes) {
        await prisma.themeConfig.upsert({
            where: { organizationId_name: { organizationId: null, name: theme.name } },
            update: theme,
            create: theme,
        });
    }
    ...
    for (const tmpl of templates) {
        await prisma.contentTemplate.upsert({
            where: { organizationId_key: { organizationId: null, key: tmpl.key } },
            update: tmpl,
            create: tmpl,
        });
    }
    ...
    for (const flag of flags) {
        await prisma.featureFlag.upsert({
            where: { organizationId_key: { organizationId: null, key: flag.key } },
            update: flag,
            create: flag,
        });
    }
